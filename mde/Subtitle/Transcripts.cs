	...
	internal sealed class TranscriptListFetcher
    {
        ...
        internal static JsonElement extractCaptionsJson( string html , string videoId )
        {
            ...
            var captions_json = JsonSerializer.Deserialize<JsonElement>(
                splitted_html[1].Split(new string[] { ",\"videoDetails" } , StringSplitOptions.None )[0].Replace("\n", "")
        ...
        {
            string debug_url = string.Format( "https://www.youtube.com/watch?v={0}" , videoId);
    ...
    internal class TranscriptParser
    {
        ...
        public IEnumerable<TranscriptItem> Parse(string plain_data)
        {
            foreach (var xmlElement in XDocument.Parse(plain_data).Root.Elements("text"))
            {
                ...
                _ = decimal.TryParse(xmlElement.Attribute(XName.Get("start")).Value, out decimal start);
                _ = decimal.TryParse(xmlElement.Attribute(XName.Get("dur"))?.Value ?? "0.0", out decimal duration );

                yield return new TranscriptItem()
                {
                    Text = text,
                    Start = (long)( start * 1000),
