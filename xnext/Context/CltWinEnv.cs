		...
		public static void SetupEnvironment( string identifier )
		{
			if( string.IsNullOrEmpty( Info._mainIdentifier ) )
				lock( Info )
				{
					...
					UserSetting.SetData( "Configuration" , "Publish Path" , Directory.CreateDirectory( Path.Combine( Info._appSetting.DirectoryName , "Publish" ) ).FullName );
					...
					MemoryCache.Default.Add( "PublishPath" , UserSetting.GetData( "Configuration" , "Publish Path" ) , Policy );
